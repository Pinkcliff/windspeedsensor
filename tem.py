...
# --------------------------
# 优化模式：单次连接+持续读取+数据变化高亮+异常处理（12路RTC温度模块专用）
# --------------------------
def single_connect_continuous_read_rtc_12ch():
    # 1. 设备参数（务必与设备手册一致！）
    DEVICE_IP = "192.168.1.101"    # RTC模块IP地址
    DEVICE_PORT = 8234           # 设备Modbus端口
    SLAVE_ADDR = 1               # 设备从站地址
    FUNC_CODE = 0x04             # 功能码（0x03=保持寄存器，0x04=输入寄存器）
    START_REG = 0                # 12路温度起始寄存器地址
    REG_COUNT = 12               # 读取12路温度寄存器数量
    ...
    print("="*80)
    print("🚀 启动 [12路RTC温度模块] 模式（单次连接+持续读取）")
    print("="*80)
        ...
        while True:
            ...
            try:
                ...
                for i in range(12):
                    ...
                    temp_raw_values.append(temp_raw)
...
if __name__ == "__main__":
    single_connect_continuous_read_rtc_12ch()