...
class RTDWindDensityInteractive:
    ...
    def update(self, frame):
        ...
        current_time = time.time() - self.start_time if self.simulation_mode else frame * 0.1
        ...
        for i in range(4):
            ...
            # Update line data (only if visible)
            label = f'{self.sensor_names[i]} Raw'
            if label in self.lines and self.lines[label].get_visible():
        ...
        for label, line in self.lines.items():
            if line.get_visible() and len(line.get_xdata()) > 0:
                all_visible_data.extend(line.get_ydata()[-50:])  # Last 50 points
        ...
        # Update title
        mode_text = "Simulation" if self.simulation_mode else "Real-time"
        if len(self.density_data) > 0:
            ...
            self.fig.suptitle(
                f'RTD Wind Speed Monitoring - Interactive Combined View ({mode_text}) | Time: {current_time:.0f}s\n'
